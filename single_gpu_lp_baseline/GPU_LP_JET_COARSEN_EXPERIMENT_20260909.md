...
支持 `METHOD=lp|basc|basc_gpu`、`BASC_K=1|2|4`、`STOP_RATIO` 和 `MAX_LEVELS`。
...
## 2026-09-09 device-resident BASC 实现状态

代码已转移到 sibling project `partition/homework`；本节只记录实验结果，产物仍在
本目录。新增 `coarsen_method=basc_gpu`，其当前层图跨层驻留 GPU，使用 warp-per-
vertex 的 anchor/candidate/support/expansion kernel，以及 GPU sort/reduce contraction
和 CSR 构造。默认 `lp` 路径仍通过同一可执行文件，kim2 严格小图回归以及 products
和 LiveJournal 的既有 BASC 诊断均未被删除。

严格小图测试已通过：输入包含自环、平行边、孤立点和断开分量，检查 K=1/2/4 的
映射覆盖、容量、双向边权、粗图自环消除和随机投影切边守恒；默认 `lp` 的 kim2
严格回归也通过。

products/k=4/seed=0 的一次 `basc_gpu` smoke 记录在
`build-gh200/experiments/results/basc_gpu_smoke/products_k4_seed0/`：10 层、最粗
156,237 点，GPU BASC aggregate+contract 核心约 3.0 秒，层次 snapshot 约 0.245
秒，完整 Jet hierarchy export 约 60.9 秒。导出的层次交给同一个
`jet_gpu_lp_import` 后端后，原图独立校验为 cut=2,210,789、cut ratio≈0.0357392、
最大负载满足 1.10；对应同一目录中的一次 Jet 原版运行 cut=2,158,447。由于 Jet
原版此轮最粗层为 576 点，而 BASC 只到约 156k 点，这不是同等粗化规模比较。

Jet `jet_export` 变体已在
`build-gh200/experiments/results/jet_export_probe/products_k4_seed0/coarse_graphs.out`
保留完整 21 层层次，其点数从 2,449,029 逐层到 83；BASC 与 Jet 的自然停止规模
仍未对齐。因此当前结论只到“GPU contraction 路径可运行且端到端瓶颈已定位”，
不宣称粗化质量改善。

同 seed 重复性尚未成立：
`build-gh200/experiments/results/basc_repeatability/basc_gpu/summary.json` 中
kim2/k=4/seed=0 三次 `basc_gpu` 的 hierarchy SHA256 不同，层数为 21、20、20。
原因是容量 admission 的 atomicCAS 竞争顺序；候选/support 规则本身已改为 K 槽
匹配，但还没有引入确定性的 GPU 分段前缀容量预留。这个问题必须在正式多 seed
质量表之前解决或明确标注。
