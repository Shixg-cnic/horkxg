...
OUT_ROOT="${OUT_ROOT:-${ROOT}/build-gh200/experiments/results/gpu_lp_jet_compare}"
RATIO="${RATIO:-1.10}"
SEED="${SEED:-0}"
...
case "$METHOD" in
    lp|basc) ;;
    *) echo "METHOD must be lp or basc" >&2; exit 2 ;;
esac
...
for dataset in "${DATASETS[@]}"; do
    ...
    for k in 4 8 32; do
        run_dir="${OUT_ROOT}/${dataset}/k${k}/seed${SEED}/${METHOD}"
        mkdir -p "$run_dir"
        ...
        echo "=== ${dataset} k=${k} seed=${SEED} B=gpu_lp ==="
